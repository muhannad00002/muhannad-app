...
route("/premium/return",(q)=>{
  ...
  function failed(msg){
    render_(h("div.card.pad-l.center",{style:{marginTop:"40px"}},[
      h("div",{style:{fontSize:"52px",marginBottom:"10px"}},"😔"),
      h("h2",{style:{fontSize:"25px"}},"Payment not completed"),
      h("p.muted",{style:{margin:"8px auto 20px",maxWidth:"300px"}},msg||"No charge was made. You can try again whenever you're ready."),
      h("button.btn.btn-pri.btn-lg",{onclick:()=>openPaywall("default")},"Try again"),
      h("button.btn.btn-quiet",{style:{marginTop:"8px"},onclick:()=>go("/home")},"Back to app"),
    ]));
  }

  (async()=>{
    // Trust the server, not the URL. Poll /api/me briefly in case the callback
    // is still landing, and reflect whatever the server actually granted.
    let confirmed=false;
    ...
    else if(q.status==="success") failed("We couldn't confirm the payment yet. If you were charged, Premium will unlock automatically within a few minutes.");
    else failed();
  })();
